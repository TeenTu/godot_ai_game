...
`req2_test`（第二轮 REQ 整改批：显式定深（含浅水 12m 攻击 0m 水面目标）/
LOS 真方位不重复计自身机动/主动距离率独立过期判定/转向 dt 积分+饱和判定/
tick 去短路/机会记账（发射前已有更新航迹不罚、空被动扫描不扣 active 航迹）/
COAST 独立分支+纯 PN 移动靶拦截/引信 tick 不可变快照+起爆成功才结算/
Mark 全局关联 vs Shift 锁定+取消选择/TRUE 模式 Mark 峰转真方位，R2-01..10，
44 断言；World 全流程含 1500m/10kn 横向移动靶实际起爆+目标 sunk）。
