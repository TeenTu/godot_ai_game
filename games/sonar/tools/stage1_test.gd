...
func _test_motion() -> void:
	...
	t.from_dict({
		"position_east_m": 0.0, "position_north_m": 0.0,
		"course_deg": 90.0, "speed_kn": 10.0,
	})
	# 航向 90° = 正东。10kn = 5.14444 m/s。跑 1 秒 → x 增加 5.144，y 不变
	t.advance(1.0)
	_check(_approx(t.position_east_m, 5.14444, 0.01), "course 90 -> x+=5.144, got %.3f" % t.position_east_m)
...
func _test_acoustic_and_propagation() -> void:
	...
	env.from_dict({
		...
		"own_noise_base_db": 40.0, "own_noise_speed_coeff": 2.0,
		"tl_spreading_k": 20.0, "tl_absorption_alpha": 0.5, "tl_environment_loss": 2.0,
	})
	...
	ac.from_dict({"broadband_base_level_db": 150.0, "speed_noise_a": 18.0, "speed_noise_n": 2.5, "speed_noise_vref_kn": 8.0})
...
func _test_detection_behavior() -> void:
	...
	sensor.set_rng(rng)
	sensor.from_dict({
		"bearing_sigma_min_deg": 0.5, "bearing_sigma_max_deg": 5.0,
		"bearing_se0_db": 6.0, "bearing_k_sigma_db": 4.0, "detection_k_d": 3.0,
	})
	# P_d 单调：SE 越高概率越高
	...
	var s_high: float = sensor.bearing_sigma_deg(20.0)
	...
	for i in range(200):
		strong_err_sum += absf(NavUtils.angle_diff(sensor.noisy_bearing(true_b, se_strong)["bearing"], true_b))
		weak_err_sum += absf(NavUtils.angle_diff(sensor.noisy_bearing(true_b, se_weak)["bearing"], true_b))
	strong_err_sum /= 200.0
	weak_err_sum /= 200.0
	_check(strong_err_sum < weak_err_sum, "avg bearing error smaller for strong SE (%.2f < %.2f)" % [strong_err_sum, weak_err_sum])
...
func _test_reproducibility() -> void:
	...
	_check(w1.measurement_count() == w2.measurement_count(), "same measurement count: %d" % w1.measurement_count())
...
func _test_full_scenario_run() -> void:
	...
	print("  produced %d measurements over %.0fs" % [w.measurement_count(), scenario.get("duration", 30.0)])
	...
	if detected_bearings.size() > 0:
		...
		_check(absf(NavUtils.angle_diff(avg, 45.0)) < 8.0, "avg bearing ~45deg (target NE), got %.2f (n=%d)" % [avg, detected_bearings.size()])
	else:
