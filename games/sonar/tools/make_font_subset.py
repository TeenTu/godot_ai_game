# -*- coding: utf-8 -*-
"""S109 Batch 7：UI 中文字体子集生成（S109 §10.3）。

字符集 = 全部 scripts/**.gd + tools/*.gd 中**字符串字面量**里的非 ASCII
字符（注释里的数学符号如 Fᵀ/↔/💥 不需要字形，不请求）+ ASCII 可打印 +
旧子集 cmap（保底不回退）+ 显式补充表。zh_cn_ui_test AT-37 用同一规则。
源字体：Microsoft YaHei（msyh.ttc index 0）。产出：
  - assets/fonts/ui_subset.ttf       （Godot 运行时/导出版）
  - assets/fonts/ui_subset_chars.txt （字形清单，zh_cn_ui_test AT-37 用）
