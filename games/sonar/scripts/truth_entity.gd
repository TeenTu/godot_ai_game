...
func advance(dt: float) -> bool:
	...
	if turn_rate_deg_s != 0.0:
		course_deg = nav_utils.wrap360(course_deg + turn_rate_deg_s * dt)

	# 平移
	var v_ms: float = nav_utils.kn_to_ms(speed_kn)
	var next: Dictionary = nav_utils.advance_pos(
...
func from_dict(d: Dictionary) -> void:
	...
	course_deg = nav_utils.wrap360(float(d.get("course_deg", 0.0)))
