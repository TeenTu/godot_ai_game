...
//  1) DYNAMIC 摇杆在屏幕多个不同区域按下都应就地出现、不飞出屏。
...
test("boom: joystick usable at multiple on-screen regions & no runaway", async ({ browser }) => {
  ...
  const regions = [
    { p: { x: 80, y: 15 }, dx: -60, dy: 40, label: "top_right" },
    { p: { x: 15, y: 25 }, dx: 60, dy: 60, label: "top_left" },
    { p: { x: 75, y: 85 }, dx: -70, dy: -50, label: "bottom_right" },
    { p: { x: 25, y: 75 }, dx: 70, dy: -60, label: "bottom_left" },
  ];
  ...
  for (let i = 0; i < regions.length; i++) {
    ...
    // 断言：底座在触点附近(触点归一 ~0.80,0.15; 底座应在 (0.6..1.0, 0.0..0.3))
