...
> **判笔动作更新（2026-09-09）**：墨线判笔的大剑动作节奏、目标上限和独立墨迹层
> 以 `design_m9_brush_horde.md` 为准；本文其余武器框架仍有效。

> **文档状态（2026-09-07）**：机制、数据与数值章节仍有效；视觉、角色和素材章节中出现的 Bubble Captain、泡泡队长、果冻兵、糖果嘉年华及旧色板均已废弃。统一改以 `docs/art_bible_boom.md` 的百怪夜巡视觉规范执行；技能树最终数据模型以 M7 的树序数组和 `TREE_PRICES` 为准，本文早期节点图仅为历史设计。
